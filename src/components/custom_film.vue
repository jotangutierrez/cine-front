      ...
      <div class="p-5 h-full">
        ...
        <form class="md:justify-between" action="/v1/reservations" method="post"
          ...
          <div class="mb-4 px-4">
            ...
            <t-input class="w-full" v-model="film.description" name="description"></t-input>
          </div>

          <div class="mb-4 px-4">
            <label class="block font-montserrat text-gray-700 text-sm font-bold mb-2" for="username">Póster</label>
            <t-input class="w-full" v-model="film.cover_url" name="cover_url"></t-input>
          </div>

          <div class="mb-4 px-4">
            <label class="block font-montserrat text-gray-700 text-sm font-bold mb-2" for="username">Fechas</label>
            <datetime-picker class="soft-rounded" date-range="true" @updated="splitDate"></datetime-picker>
          </div>

          <div class="px-4 py-4 text-center w-full">
            <div class="pl-8">
              <t-button variant="primary" @click="onSubmit()" class="enviar md:w-1/2 rounded-full">Crear</t-button>
            </div>
          </div>
        </form>
      </div>
